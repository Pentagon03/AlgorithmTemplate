namespace Gemoetry{
    ...
    inline P operator*=(P&a,int c){ return a = c*a; }
    inline P operator-=(P&a,P b){ return a = a-b; }
    inline T sq(P a){return a.fi*a.fi+a.se*a.se;}
    ...
    struct Line{
        ...
        friend P parrallel_inter(Line p,Line q){
    ...
    Polygon getCH(const Polygon&pg){ //Monotone Chain of polygon
    ...
    bool inner_naive(const Polygon&h, P p){
        // strictly inner
        int s = ccw(h[0],h[1],p);
